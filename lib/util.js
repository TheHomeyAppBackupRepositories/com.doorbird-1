...
class Util {
  ...
  updateNotificationsCloud(oAuth2Client, webhook_id, mac, action) {
    return new Promise(async (resolve, reject) => {
      try {
        const relays = await oAuth2Client.getRelays();
        const homeyId = await this.homey.cloud.getHomeyId();
        var number_http = 1001;
        var number_schedule = 1001;

        if (action == 'remove') {
          let favorites = await oAuth2Client.getFavorites();

          for(const [id, values] of Object.entries(favorites.http)) {
            if (values.title.includes('Homey')) {
              let result = await oAuth2Client.deleteFavorite(values.identifier);
            }
          }
        } else if (action == 'add') {

          // add http callbacks for doorbell
          let favorite_doorbell = {
            "identifier": 1000,
            "title": "Homey Doorbell Trigger",
            "value": `https://webhooks.athom.com/webhook/${webhook_id}?homey=${homeyId}&event=doorbell&id=${mac}`,
            "displayedTitle": "Homey Doorbell Trigger"
          }
          let doorbell_favorite_result = await oAuth2Client.addFavorite(favorite_doorbell);

          // add http callbacks for motion
          let favorite_motion = {
            "identifier": 1001,
            "title": "Homey Motion Trigger",
            "value": `https://webhooks.athom.com/webhook/${webhook_id}?homey=${homeyId}&event=motionsensor&id=${mac}`,
            "displayedTitle": "Homey Motion Trigger"
          }
          let motion_favorite_result = await oAuth2Client.addFavorite(favorite_motion);

          // add http callbacks for each relay
          relays.forEach(async (relay) => {
            number_http++;
            let favorite_relay = {
              "identifier": number_http,
              "title": `Homey Relay ${relay.name} Trigger`,
              "value": `https://webhooks.athom.com/webhook/${webhook_id}?homey=${homeyId}&event=relays&relay=${relay.id}&id=${mac}`,
              "displayedTitle": `Homey Relay ${relay.name} Trigger`
            }
            let relay_favorite_result = await oAuth2Client.addFavorite(favorite_relay);
          });

          /* schedule http callbacks */
          let schedule = await oAuth2Client.getSchedules();
          let doorbell_schedule = schedule.find(obj => { return obj.input === 'doorbell' });
          let motion_schedule = schedule.find(obj => { return obj.input === 'motion' });
          let relay_schedule = schedule.filter(relay => relay.input.includes('relay'));
          ...
          if (doorbell_schedule !== undefined) {
            if (doorbell_schedule.output.some(item => item.event === 'http')) {
              ...
              let result_doorbell_schedule = await oAuth2Client.addSchedules(doorbell_schedule);
            } else {
              let http_event = {
                "event": "http",
                "param": "1000",
            ...
            let result_doorbell_schedule = await oAuth2Client.addSchedules(payload);
          }
          ...
          if (motion_schedule !== undefined) {
            if (motion_schedule.output.some(item => item.event === 'http')) {
              ...
              let result_motion_schedule = await oAuth2Client.addSchedules(motion_schedule);
            } else {
              let http_event = {
                "event": "http",
                "param": "1001",
            ...
            let result_motion_schedule = await oAuth2Client.addSchedules(payload);
          }
          ...
          if (relay_schedule.length === 0) {
            relays.forEach(async (relay) => {
              ...
              let result_relay_schedule = await oAuth2Client.addSchedules(payload);
            });
          } else {
            relays.forEach(async (relay) => {
              number_schedule++
              let current_schedule = relay_schedule.filter(item => item.param.includes(relay.id));
              if (current_schedule.length > 0) {
                if (current_schedule[0].output.some(item => item.event === 'http')) {
                  current_schedule[0].output.filter(e => e.event === 'http').forEach(e => {
                    e.param = ""+ number_schedule +"";
                  ...
                  let result_relay_schedule = await oAuth2Client.addSchedules(current_schedule[0]);
                } else {
                  let http_event = {
                    "event": "http",
                    "param": ""+ number_schedule +"",
